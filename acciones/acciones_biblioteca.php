<?php
    ...
    /* Función para mostrar los elementos de la biblioteca de un usuario */
        ...
        try { /* Inicio bloque try para capturar errores */
            ...
            if(isset($_SESSION['datos_busqueda']) && isset($_SESSION['datos_busqueda']['juegos_encontrados'])) {
                ...
                $consulta = $conexion->prepare("
                    ...
                    FROM biblioteca b
                    INNER JOIN juegos j ON b.id_juego = j.id AND b.id_usuario = ?
                    WHERE j.id IN ($cadena)
                    ORDER BY j.tipo DESC, b.fecha_adquisicion DESC
                "); /* Preparo consulta para obtener los juegos encontrados que estén en la biblioteca del usuario ordenados primero por tipo y luego por fecha */
